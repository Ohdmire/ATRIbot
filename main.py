...
async def app_lifespan(app: FastAPI):
    ...
    scheduler.add_job(
        ATRIproxy.cleanup_brk_cache, trigger="interval", hours=1
    )  # 定时清理brk缓存
    scheduler.start()
    yield
    logging.info("亚托莉，关闭!")
...
async def fetch_profile(item: IName):
    img_bytes = await ATRIproxy.format_profile(
        item.qq_id, item.osuname, item.vs_qq_id, item.is_yesterday
    )
    if type(img_bytes) is BytesIO:
        return StreamingResponse(img_bytes, media_type="image/jpeg")
    else:
...
async def fetch_profile_monitor(item: ItemN):
    result = await ATRIproxy.format_monitor_profile(
        item.group_id, item.group_member_list
    )
    return str(result)
...
async def job_fetch_group_info(item: ItemN):
    result = ATRIproxy.format_job_update_group_list(
        item.group_id, item.group_member_list
    )
    return str(result)
...
async def fetch_brk_up(item: IName):
    ...
    result = await ATRIproxy.format_brk_up(
        item.qq_id, item.osuname, item.beatmap_id, item.group_id
    )
...
async def fetch_brk(item: IName):
    img_bytes = await ATRIproxy.format_brk(
        item.qq_id,
        item.osuname,
        item.beatmap_id,
        item.group_id,
...
async def fetch_bd(item: IName):
    img_bytes = await ATRIproxy.format_bd(
        item.qq_id, item.osuname, item.beatmap_id, item.group_id, item.mods_list
    )
    if type(img_bytes) is BytesIO:
        return StreamingResponse(img_bytes, media_type="image/jpeg")
    else:
...
async def jobs(item: IName):
    img_bytes = await ATRIproxy.format_brkpr(
        item.qq_id, item.osuname, item.group_id, item.index, item.is_old
    )
    if type(img_bytes) is BytesIO:
        return StreamingResponse(img_bytes, media_type="image/jpeg")
    else:
...
async def fetch_replay_similarity(item: IName):
    result = await ATRIproxy.format_replay_similarity(
        item.qq_id, item.vs_qq_id, item.osuname, item.vsname
    )
    return str(result)
...
async def fetch_joindate(item: IName):
    result = await ATRIproxy.format_joindate(
        item.qq_id, item.group_id, item.osuname, item.pp_range, item.group_member_list
    )
    return str(result)
...
async def fetch_avgstar(item: IName):
    img_bytes = await ATRIproxy.format_avgstar(
        item.qq_id, item.osuname, item.pp_range, item.star_min, item.star_max
    )
    if type(img_bytes) is BytesIO:
        return StreamingResponse(img_bytes, media_type="image/jpeg")
    else:
...
async def fetch_changelog_draw(item: ItemN):
    async with changelog_draw_route_semaphore:
        img_bytes = await ATRIproxy.format_changelog_draw(
            item.stream_name, item.index, item.cache
        )
        if type(img_bytes) is BytesIO:
            return StreamingResponse(img_bytes, media_type="image/jpeg")
        else:
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app", host="0.0.0.0", port=8008, reload=True, timeout_keep_alive=120
    )
