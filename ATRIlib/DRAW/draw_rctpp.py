...
import logging
import random
from ATRIlib.DRAW.draw_interbot import draw_data, drawRank
...
async def drawRec(mapjson, recinfo, bestinfo, userjson, debug=0, **kw):
    """
    kwargs:
        pp, fcpp, acpp
    ...
    me_avatar_url =  userjson.get('avatar_url', '')
    avatar_img = avatar_path / \
                 f'{me}.jpeg'
    if avatar_img.exists() is True:
        pass
    else:
        await Download.download_avatar_async([me_avatar_url],
                                             [me])
    ...
    acc = float(bestinfo['accuracy'])*100
    ...
    # acc
    acc2 = float(recinfo['accuracy'])*100
