...
def update_beatmap_type(user_id, beatmap_type):
    db_beatmaptype.update(
        {"id": user_id},
        {"$set": {"beatmap_type": beatmap_type}},
        upsert=True
    )
...
def update_beatmap_attributes(beatmap_id, attributes):
    ...
    if all(value == 0 for value in update_data.values()):
        ...
        db_beatmaptype.delete_one({"beatmap_id": beatmap_id})
    else:
        # 否则，更新或插入文档
        db_beatmaptype.update_one(
...
def get_beatmap_attributes(beatmap_id):
    result = db_beatmaptype.find_one({"beatmap_id": beatmap_id})
    if result:
        return {
